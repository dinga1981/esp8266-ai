...
> 当前正式增强版：`v0.5.12`；当前稳定性测试版：`v0.5.17-test`。本项目从
...
- **Codex 状态与额度**：Nothing 风格点阵数字、5 小时/周额度、准确重置日期时间、重置倒计时和等待审批提醒。
- **四行报价**：一页最多显示 4 个 A 股、港股、美股、国内外期货、伦敦金、离岸人民币或美元指数。
- **全屏 K 线**：支持 1、5、60 分钟周期，最多收藏 15 个标的，支持添加和删除收藏。
- **低内存K线传输（v0.5.12）**：16色固定颜色表与4位RLE保留红涨绿跌，并让复杂行情帧也能在ESP8266上完整校验后切屏。
- **多市场行情**：BTC/ETH、A 股、港股、美股、韩股、主要指数、国内期货、海外期货、伦敦金、离岸人民币和美元指数。
- **稳定轮换**：K 线下一标的提前加载，完整校验后一次切屏；接口失败时跳过，不拖住整个轮换。
- **日期天气**：中文大字城市、月日星期、时分秒、当前天气与温度，以及中文的今日和明日预报。
- **按星期自动轮播**：工作日和周末可分别选择页面及 5/10/15/20/30/60/120 秒间隔；进入 K 线页后会等待收藏完整展示一轮。
- **音乐和桌宠**：保留上游音乐显示、自定义 GIF 桌宠、亮度控制和实时屏幕预览。
- **设置备份恢复（v0.5.9）**：一个 JSON 文件同时保存 Mac 行情/天气设置和设备显示、亮度、轮播设置；不包含 Wi-Fi 密码或登录凭据。
- **网络 OTA（v0.5.9）**：第一次刷入支持 OTA 的固件后，可从设备网页直接上传后续 ESP8266 固件，不擦除 LittleFS、Wi-Fi 和桌宠文件。
- **设备网页诊断（v0.5.9）**：设备网页显示 Wi-Fi 信号、桥接状态、数据年龄、两端版本、可用内存、文件系统、运行时间和 OTA 空间，不占用屏幕轮播页面。
- **断线根因诊断（v0.5.12）**：区分首次/最后断线及固件主动恢复事件，保存RSSI、信道、BSSID、恢复时间线和最近4次重启摘要。
- **网络稳定性增强（v0.5.15-test）**：所有设备外发请求统一排队并增加请求间冷却；降低行情元数据与状态轮询频率，压缩四行报价中文名称传输，并提供 ESP8266 Core 3.1.2/3.0.2 两种固件用于定位底层 Wi-Fi 看门狗问题。
- **天气本地缓存（v0.5.16-test）**：中文天气文字使用带CRC校验的RLE协议，仅在内容版本变化时下载并原子缓存到LittleFS；页面轮播直接本地重绘，不再反复传输约30KB画面。
- **四行报价名称缓存（v0.5.17-test）**：中文名称图仅在首次使用或自选名称变化时下载，之后由设备从 LittleFS 本地重绘；稳定内容版本、CRC 校验、原子替换和失败退避避免重复传输及损坏缓存。

`v0.5.8` 的菜单中暂时隐藏 Claude 页面和网速页面，但底层代码仍保留。增强功能目前仅适配
...
在 Mac 菜单栏图标上点右键，选择“设置备份与恢复”。导出的 JSON 包含行情收藏、K 线周期、天气城市、设备亮度、显示模式和两套轮播方案。恢复前先配对目标设备；恢复时会自动把设备桥接地址改为当前 Mac。

### 网页 OTA

打开设备网页，在“网络 OTA 固件升级”中选择本项目构建的 `firmware.bin`。只有先通过 USB 或 Web Flasher 安装一次 v0.5.9 或更高版本，后续才能使用 OTA。上传和重启期间不要断电；正常 OTA 不擦除 Wi-Fi、设置和LittleFS。

如果设备正在运行 `v0.5.10-test`，请使用USB Web Flasher升级一次：该测试版的RTC诊断区域与OTA引导指令冲突，网页上传后只会重启旧固件。`v0.5.11-test`及之后版本已修复，能够直接网页OTA。

macOS 首次阻止打开时，可在“系统设置 → 隐私与安全性”中选择允许。

### 稳定性诊断（v0.5.15-test）

桥接程序提供 `http://127.0.0.1:8765/health`，设备网页的“设备诊断”会显示当前/上次网络任务、请求阶段、HTTP结果、耗时、数据量和重启历史。Mac 端诊断文件位于：

```text
~/Library/Logs/AIClockBridge/access.log
~/Library/Logs/AIClockBridge/boot-reports.jsonl
```

两个文件达到 2 MB 后自动轮换，只保留请求路径、状态、耗时和设备上报的重启诊断，不保存响应正文、Wi-Fi 密码或自选设置。也可以运行 `tools/monitor_stability.sh 设备IP` 连续观察设备与桥接健康状态。

测试包内有两份功能相同的固件。建议先连续运行 `core-3.1.2` 标准版至少 24 小时；若仍发生看门狗重启，再刷入 `core-3.0.2` 对照版并保持相同页面、轮播和网络环境。设备诊断中的“核心构建”可用于确认当前版本，切换固件不会清除 Wi-Fi、收藏或轮播设置。
...
默认环境为 `nodemcuv2`，4 MB Flash 使用支持 OTA 的双固件布局。实际刷写前请确认屏幕引脚和 Flash 布局与目标硬件一致；OTA 页面只接受 ESP8266 `.bin` 固件。
